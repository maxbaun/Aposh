...
define('filters',['jquery','imagesloaded','isotope','jquery-bridget'],function($,imagesLoaded,Isotope,jQueryBridget ){
    var ajaxurl = $("meta[name=ajaxurl]").first().attr("content");
    ...
    $.aposhFilter = function(element, options) {
        ...
        var $header   = $(element).find('.filter-header').first();
        ...
        var resetMasonry = function(){
          ...
          if($(plugin.settings.contentWrapper).data('infinitescroll') && $(plugin.settings.contentWrapper).data('infinitescroll') !== 'undefined'){
            ...
            if($(plugin.settings.pageNavSelector).find('a').length > 0){
              $(plugin.settings.pageNavSelector).find('a').attr('href',$nextPage);
            }
            else{
            ...
            $(plugin.settings.contentWrapper).data('infinitescroll').options.loading.msg.find('div').html($(plugin.settings.contentWrapper).data('infinitescroll').options.loading.msgText);
            $(plugin.settings.contentWrapper).data('infinitescroll').options.loading.msg.find('img').show();
            ...
            $(plugin.settings.contentWrapper).infinitescroll(isSettings,
        ...
        var itemClicked = function(e) {
          ...
          $selectedFilter.addClass('invisible');
        	$selectedFilter.appendTo($header).hide().show();
